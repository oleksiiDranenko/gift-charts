...
export default function CompareCharts({
  ...
  useEffect(() => {
    ...
    switch (listType) {
      case "1m":
        newList = lifeData.map((data, index) => [
          ...data.slice(-30),
          ...(weekData[index]?.slice(-1) || []),
        ]);
        break;
      case "all":
        newList = lifeData.map((data, index) => [
  ...
  const labels =
    list[index]?.map((item: any) =>
      listType === "24h" && item.time
        ? item.time.slice(0, 5)
        : item.date.slice(0, 5)
    ) || [];
        ...
        <div className='w-full flex flex-row justify-between gap-x-3'>
          <button
            className={`w-full text-sm h-10 ${
              listType === "all" ? "rounded-3xl bg-primary font-bold" : ""
            }`}
            onClick={() => {
          ...
          <button
            className={`w-full text-sm h-10 ${
              listType === "1m" ? "rounded-3xl bg-primary font-bold" : ""
            }`}
            onClick={() => {
          ...
          <button
            className={`w-full text-sm h-10 ${
              listType === "1w" ? "rounded-3xl bg-primary font-bold" : ""
            }`}
            onClick={() => {
          ...
          <button
            className={`w-full text-sm h-10 ${
              listType === "24h" ? "rounded-3xl bg-primary font-bold" : ""
            }`}
            onClick={() => {
