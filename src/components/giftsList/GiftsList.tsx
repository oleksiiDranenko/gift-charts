...
export default function GiftsList({ loading }: PropsInterface) {
            ...
            switch (filters.sortBy) {
                ...
                case 'marketCap':
                    sortedList.sort((a, b) =>
                        filters.currency === 'ton'
                            ? filters.sort === 'lowFirst'
                                ? a.priceTon * a.supply - b.priceTon * b.supply
                                : b.priceTon * b.supply - a.priceTon * a.supply
                            : filters.sort === 'lowFirst'
                            ? a.priceUsd * a.supply - b.priceUsd * b.supply
                            : b.priceUsd * b.supply - a.priceUsd * a.supply
                    );
                    break;
                case 'supply':
                    sortedList.sort((a, b) =>
        ...
        const filteredList = giftsList.filter((gift) => {
            return (
                gift.name
                    ...
                    .replace(/[^a-zA-Z0-9]/g, '') ===
                    value.toLowerCase().trim().replace(/[^a-zA-Z0-9]/g, '') ||
                gift.name
                    .toLowerCase()
                    .replace(/[^a-zA-Z0-9]/g, '')
                    .includes(value.toLowerCase().trim().replace(/[^a-zA-Z0-9]/g, ''))
            );
        });
    ...
    }, [value, giftsList, dispatch, filters]);
                ...
                <>
                        ...
                        <div className="w-full h-auto pt-3">
                            <div className="w-full flex flex-row justify-between items-center mb-5 gap-x-3">
                                <div className="w-1/2 gap-2 flex justify-between">
                                    <button
                                        className={`w-1/2 text-sm h-10 box-border rounded-lg ${
                                            filters.currency === 'ton'
                                                ? 'bg-[#0098EA] font-bold'
                                                : 'bg-slate-800'
                                        }`}
                            ...
                            <div className="w-full flex flex-row justify-end items-center mb-5 gap-x-3">
                                ...
                                <div className="w-1/2 flex justify-between items-center">
                                    <span className="text-slate-500 mr-2 text-sm">Value:</span>
                                    <select
                            ...
                            <div className="w-full flex flex-row justify-between gap-x-3">
                                <button
                                    className={`w-full text-sm h-10 ${
                                        timeGap === '1m' ? 'rounded-lg bg-[#0098EA] font-bold' : ''
                                    }`}
                                    onClick={() => {
                                        setTimeGap('1m');
                                        vibrate();
                                ...
                                <button
                                    className={`w-full text-sm h-10 ${
                                        timeGap === '1w' ? 'rounded-lg bg-[#0098EA] font-bold' : ''
                                    }`}
                                    onClick={() => {
                                        setTimeGap('1w');
                                        vibrate();
                                ...
                                <button
                                    className={`w-full text-sm h-10 ${
                                        timeGap === '24h' ? 'rounded-lg bg-[#0098EA] font-bold' : ''
                                    }`}
                                    onClick={() => {
                                        setTimeGap('24h');
                                        vibrate();
                    ...
                    <div className="w-full mt-2 flex flex-row items-center justify-between h-6 text-xs text-slate-500">
                        ...
                        <div className="">
                            ...
                            {timeGap === '24h' ? '24h ' : timeGap === '1w' ? '1w ' : '1m '}change
                        </div>
                    </div>
